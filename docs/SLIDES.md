...
>
> **קובץ מצגת מוכן:** [DefiNet-Defense-Deck.pptx](DefiNet-Defense-Deck.pptx) — 6 שקפים בעברית (RTL).
> נותר למלא בו: שמות המבצעים, כתובת ה-GitHub וכתובת הענן (שקף 1) — לאחר הפריסה.
...
3. **MongoDB מקומי** (mongodb-memory-server) שומר נתונים בתיקייה מקומית מתמידה;
   מחיקת `server/data` מאפסת את המערכת לנתוני הדמו. בענן משתמשים ב-MongoDB Atlas.
